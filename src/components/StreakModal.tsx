    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[100] bg-[#0a0a0a] overflow-y-auto selection:bg-orange-500/30"
        >
          <div 
            className="min-h-screen relative w-full flex flex-col items-center overflow-hidden"
            style={{ 
              background: `radial-gradient(circle at 50% -20%, ${currentStage.level >= 10 ? '#1e40af' : currentStage.level >= 5 ? '#9a3412' : '#ea580c'}44, #0a0a0a 70%)` 
            }}
          >
            ...
            <div className="w-full max-w-xl px-6 pb-24 flex flex-col items-center relative z-10">
              ...
              <div className="relative w-full flex flex-col items-center mb-4">
                ...
                <div className="mt-2 px-2 py-0.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md">
                  <span className="text-[8px] font-black text-white/40 tracking-[0.2em] uppercase">{currentStage.name}</span>
                </div>
              </div>
              ...
              >
                ...
                <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.3em] mb-4">سلسلة أيامك</p>
                <div className="flex items-center justify-center gap-4">
                  ...
                  <h2 className="text-6xl md:text-8xl font-black text-white tracking-tighter drop-shadow-[0_0_40px_rgba(255,255,255,0.2)]">
                    {stats.currentStreak}<span className="text-white/20 text-3xl md:text-4xl ml-1">/30</span>
                  </h2>
                </div>
                ...
                <div className="mt-10 w-full">
                  <div className="flex justify-between items-end mb-2 px-1">
                    <span className="text-[9px] font-black text-white/30 uppercase tracking-widest">التقدم للمظهر التالي</span>
              ...
              <div className="w-full mt-10 space-y-4">
                <div className="flex items-center justify-between px-4 mb-2">
                  <h4 className="text-white text-lg font-black">المهام اليومية</h4>
                  ...
                  {tasks.map((task) => (
                    ...
                    >
                      ...
                      <div className="flex-1 text-right">
                        <p className={`text-base font-bold ${task.completed ? 'text-white/30 line-through' : 'text-white'}`}>
                          {task.title}
                        </p>
                      ...
                      <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all ${task.completed ? 'bg-emerald-500 border-emerald-500 text-white scale-110' : 'border-white/10 group-hover:border-white/30'}`}>
              ...
              <div className="w-full mt-10 grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-6 flex flex-col items-center text-center group hover:bg-white/[0.04] transition-colors">
                    <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center text-white/40 mb-3 group-hover:text-white/80 transition-colors">
                      {stat.icon}
                    </div>
