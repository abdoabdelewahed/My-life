...
const { 
  Heart, Wind, ChevronRight, ArrowLeft, ArrowRight, RotateCcw, Brain, X, Target, Activity, CheckCircle2
} = Lucide;
import { playPop, playLevelUp } from '../utils/sounds';
import AssessmentFlow from './AssessmentFlow';
import { Button } from './ui/Button';
import { HEALTH_ASSESSMENTS, Assessment, Question, SubCategoryDef } from '../data/healthAssessmentsData';
...
const getLevel = (score: number, type: 'positive' | 'negative' = 'positive') => {
  ...
  if (type === 'positive') {
    ...
    return { label: 'يحتاج طبيب', color: 'red' };
  } else {
...
export default function HealthAssessmentPage() {
  ...
  const [results, setResults] = useState<Record<string, AssessmentResult>>(() => {
    try {
      const saved = localStorage.getItem('healthAssessmentsResults');
  ...
  useEffect(() => {
    localStorage.setItem('healthAssessmentsResults', JSON.stringify(results));
  }, [results]);
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 relative z-10">
          <div className="w-full max-w-lg mx-auto text-center">
            ...
            <div className="w-full space-y-3 mb-8">
              <h3 className="text-lg font-black text-white text-right mb-3">تحليل الصحة</h3>
            ...
            <div className="max-w-3xl mx-auto flex gap-4">
              ...
              {resultStep < totalSteps - 1 ? (
                ...
                <Button
                  ...
                  variant="primary"
                  className="flex-1 h-14 rounded-2xl font-black text-lg transition-all shadow-xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-500/20"
                >
                  إنهاء
  ...
  if (view === 'assessment' && activeAssessment) {
    return <AssessmentFlow assessment={activeAssessment} onClose={() => setView('dashboard')} onComplete={finishAssessment} />;
  }

  if (view === 'result_summary' && activeAssessment) {
    return renderResultSummary();
  }

  if (view === 'results' && activeAssessment) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6 md:mb-10 relative">
        <div className="inline-block px-3 py-1 md:px-4 md:py-1.5 mb-3 md:mb-4 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-400 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] transition-colors duration-300">
          اكتشف صحتك
        </div>
        <h2 className="text-2xl md:text-4xl font-black text-gray-900 dark:text-white mb-2 md:mb-4 tracking-tighter leading-tight transition-colors duration-300">
          تقييم <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-600 to-orange-600 dark:from-rose-400 dark:to-orange-500">أعضاء الجسم</span>
        </h2>
        <p className="text-gray-500 dark:text-gray-400 text-xs md:text-base font-medium max-w-xl mx-auto leading-relaxed px-4 md:px-0 transition-colors duration-300">
          قس كفاءة أعضاء جسمك وتعرف على الإجراءات اللازمة للحفاظ على صحتك.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
        {HEALTH_ASSESSMENTS.map((assessment, idx) => {
              ...
              {isCompleted ? (
                <motion.div
                  ...
                  onClick={() => {
                    ...
                    setResultStep(0);
                    const result = results[assessment.id];
                    if (result && result.subCategories.length > 1) {
                      setView('result_summary');
                    } else {
                      setView('results');
                    }
