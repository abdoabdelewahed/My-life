...
import { WORK_ASSESSMENTS, Assessment } from '../data/workAssessmentsData';
...
export default function WorkAssessmentPage() {
  ...
  const [results, setResults] = useState<Record<string, AssessmentResult>>(() => {
    const saved = localStorage.getItem('workAssessmentResults');
    return saved ? JSON.parse(saved) : {};
  });
  const [currentResult, setCurrentResult] = useState<AssessmentResult | null>(null);

  useEffect(() => {
    localStorage.setItem('workAssessmentResults', JSON.stringify(results));
  }, [results]);
  ...
  const renderDashboard = () => (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {WORK_ASSESSMENTS.map((assessment, idx) => {
